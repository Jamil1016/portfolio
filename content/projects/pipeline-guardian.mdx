...
- **Security: the minimal tool surface is the whole security posture.** Constraining the LLM to a fixed set of typed function calls eliminates entire attack classes upstream — no SQL string injection, no shell escape, no arbitrary action surface. The deterministic precondition guard then ensures the LLM is never the final authority on a destructive operation.
